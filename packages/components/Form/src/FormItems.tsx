import { ElInput, ElFormItem,ElTooltip,ElIcon, ElAutocomplete, ElCol , ElInputNumber,ElSelect , ElOption} from "element-plus";
import {  defineComponent,h,  resolveComponent } from "vue";
import { customComponentSlot, elFormItemSlot, getElComponentSlot } from "./slot";
import { FormItemComponentName,  PartialFormSchema } from './types'


// 不同组件调用不同策略接口
const componentStrategy: Partial<Record<FormItemComponentName, (schema: PartialFormSchema)=> void>>  = {
    Autocomplete(schema){
        const apiFn = schema.api;
        schema.fetchSuggestions = async (queryString: string, cb: (arg: any) => void) => {
...
export default defineComponent({
    ...
    setup(props, { emit }){
        ...
        const MyComponent = resolveComponent('El' + schema.component) as string;

        // 不同组件可能需要不同的动作，例如远程搜索等，这里使用策略模式，后期只要componentStrategy加上不同的策略名跟方法就能扩展各组件的差异
        schema.component && componentStrategy[schema.component] && (componentStrategy[schema.component])!(schema);
        ...
        // 获取自定义插槽组件
        const { componentBottom, top,bottom,labelLeft,labelRight,append,before } = customComponentSlot(schema, slots);
        ...
        return () => (
            <>
                ...
                <el-col {...schema.colProps} style={colRow} >
                    { top() }
                    <el-form-item v-slots={ elFormItemSlot(schema, labelLeft, labelRight) } labelWidth={ schema.labelWidth } prop={ schema.prop }>
                        {getElComponent()}
                        {componentBottom()}
                    </el-form-item>
                    { bottom() }
