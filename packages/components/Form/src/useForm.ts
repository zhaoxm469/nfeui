...
import { FormItemDefault } from "./config";
...
export default function useForm(
    ...
    formProps.formItems?.forEach((item) => {
        if (!item.component && !item.customSlot) console.error(`请检查formItems，${item.label}:${item.prop}缺少component字段！`)
        ...
        // 设置默认值
        Object.assign(FormItemDefault, item)
