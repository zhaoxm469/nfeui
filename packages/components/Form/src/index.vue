<template>
    <div class="nfe-form">
        <el-form ref="ruleFormRef" v-bind="formPropsRef" :model="formModel" :rules="rules">
            <el-row>
            ...
            <el-row :span="24" v-if="showFootBtn" align="middle" justify="center">
                <el-button
                    ...
                    @click="onSubmit"
                >确定</el-button>
                <el-button v-if="formPropsRef.showResetButton" @click="onResetFields">重置</el-button>
                <el-button type="primary" v-if="formPropsRef.showMockButton" @click="onMock">模拟数据</el-button>
                <el-button v-if="formPropsRef.showCancelButton" @click="onCancel">取消</el-button>
            </el-row>
        </el-form>
    </div>
</template>
...
import { computed, defineComponent, onMounted, toRefs, reactive, ref, watch, toRaw } from 'vue';
import { FormActionType, FormProps, FormSchema, FormSubmitParams } from './types';
...
interface State {
    ...
    formItemSchema: Partial<FormSchema>[] | undefined,
    ...
    setup(props, { emit, slots }) {
        ...
        const state: State = reactive({
            ...
            showFootBtn: computed(() => {
                const { showCancelButton, showMockButton, showResetButton, showSubmitButton } = state.formPropsRef
                return showCancelButton || showMockButton || showResetButton || showSubmitButton
            }),
            rules: computed(() => {
        ...
        watch(() => state.formItemSchema, (newFormItemSchema) => {
            toRaw(newFormItemSchema)?.forEach(item => state.formModel[item.prop!] = item.value || '')
        }, {
            deep: true
        })
            ...
            async onMock() {
                state.formItemSchema?.forEach(item => {
                    if (item.mock && (item.mock.type || item.mock.rules)) {
                        const mockData = item.mock.type ? getTyperData(item.mock.type) : getRulesData(item.mock.rules)
                        item.value = mockData
                    }
                })
            },
